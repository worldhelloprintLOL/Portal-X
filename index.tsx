      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-96">
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 rounded-full py-2 px-10 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-all text-white placeholder-slate-600"
          />
    ...
    <div className="space-y-16">
      {!searchQuery && (
        <section className="relative h-[450px] md:h-[550px] rounded-[2rem] overflow-hidden group shadow-2xl">
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-8 md:px-20 max-w-3xl gap-6">
            <span className="bg-indigo-600 text-white text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] shadow-lg shadow-indigo-600/20 w-fit">Featured Game</span>
            ...
            <div className="flex gap-4 mt-4">
              ...
              <button className="bg-slate-800/80 hover:bg-slate-700 backdrop-blur-md text-white px-8 py-3 rounded-xl font-bold transition-all active:scale-95">Learn More</button>
            </div>
          </div>
        </section>
      )}
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          ...
          {filtered.length === 0 && (
            <div className="col-span-full text-center py-32 bg-slate-900 border border-slate-800 rounded-3xl">
              <h3 className="text-2xl font-bold text-slate-500">No games found</h3>
              <p className="text-slate-600 mt-2 uppercase text-xs font-black tracking-widest">Try a different search query</p>
            </div>
          )}
        </div>
      </section>
...
const GameDetail = () => {
  ...
  if (!game) return <div className="text-center py-20 text-white font-black text-3xl uppercase tracking-tighter">Game Not Found</div>;
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 space-y-8">
          ...
          <div className="p-8 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-xl">
            <h2 className="text-xl font-black text-white mb-4 uppercase tracking-wider">About</h2>
            <p className="text-slate-400 leading-relaxed font-medium">{game.description}</p>
          </div>
        </div>
        <aside className="w-full lg:w-96 space-y-6">
          <h2 className="text-xl font-bold text-white flex items-center gap-2"><span className="w-1.5 h-6 bg-indigo-500 rounded-full" />Recommended</h2>
          {games.filter(g => g.id !== id).slice(0, 4).map(g => (
            <Link key={g.id} to={`/game/${g.id}`} className="flex items-center gap-4 bg-slate-900/50 p-3 rounded-xl hover:bg-slate-900 border border-transparent hover:border-slate-800 transition-all group">
              <img src={g.thumbnail} alt={g.title} className="w-20 h-20 rounded-lg object-cover" />
              <div className="flex-1 min-w-0"><h4 className="font-bold text-white truncate text-sm group-hover:text-indigo-400">{g.title}</h4><p className="text-xs text-slate-500">{g.category}</p></div>
            </Link>
          ))}
        </aside>
      </div>
